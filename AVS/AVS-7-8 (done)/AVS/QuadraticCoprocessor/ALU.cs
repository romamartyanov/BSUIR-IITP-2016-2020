	...
	public class ALU
	{
		...
		public string AddInt(string op1, string op2)
		{
			...
			for (int i = len - 1; i >= 0; i--)
			{
				...
				rem = cur / 2;
				res += cur % 2;
				if (cur % 2 == 1 && running == "" && print)
				{
					Console.WriteLine(Interpreter.Normalize(res, 23) + "\t|\tStep " + (op2.Length - i).ToString() +
						", Cur res=" + Interpreter.Normalize(new string(res.ToCharArray().Reverse().ToArray()), 23).TrimStart('0'));
				}
			}
		...
		public string Add(string op1, string op2, ref Dictionary<string, bool> flags)
		{
			...
			if (overflow)
			{
				if (op1[0] == op2[0])
				{
					if (print) Console.WriteLine("Normalizing exponent: ");
			...
			overflow = false;
			res = sgn + exp + res;
			if (print) Console.WriteLine(res);
			return res;
		}
		...
		public string Sub(string op1, string op2, ref Dictionary<string, bool> flags)
		{
			char sgn = '0';
			if (op2[0] == '0') sgn = '1';
			return Add(op1, sgn + op2.Substring(1, op2.Length - 1), ref flags);
		}
		...
		public string Mul(string op1, string op2, ref Dictionary<string, bool> flags)
		{
			...
			string mant1 = op1.Substring(9, 23), mant2 = op2.Substring(9, 23);
			string exp = op1.Substring(1, 8);
			res = MulInt('1' + mant1, '1' + mant2, ref flags);
			int mod = 1;
			for (int i = 0; i < res.Length; i++)
			{
				if (res[i] == '1')
		...
		public string MulInt(string op1, string op2, ref Dictionary<string, bool> flags)
		{
			running = "mul";
			if (print)
			{
			...
			for (int i = op2.Length - 1; i >= 0; i--)
			{
				if (op2[i] == '1')
				{
					...
					if (print) Console.WriteLine(Interpreter.Normalize(op1 + new string('0', op2.Length - i - 1), 23)
						+ "\t|\tStep " + (op2.Length - i).ToString() + 
						", Cur res=" + Interpreter.Normalize(res, 23).TrimStart('0'));
				}
			}
			if (print) Console.WriteLine(new string('-', 23));
			//Console.WriteLine("Answer: " + Interpreter.Normalize(res, 23).TrimStart('0'));
			running = "";
		...
		{
			running = "div";
			if (print) Console.WriteLine(op1 + "\t|\t" + op2);
			...
			for (int i = 0; i < op1.Length; i++)
			{
				...
				else
				{
					...
					if (print) Console.Write(new string(' ', i - tmpTrimmed.Length + 1) + op2.TrimStart('0') + '\n');
